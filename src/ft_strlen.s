...
; preliminary initialization
	...
	mov eax, edi
; check whether the first 32 bytes are on the same page
	and eax, 4095 ; modulo 4096
	cmp eax, 4064 ; 4096 - 32
	ja .first_32_bytes_are_split_on_2_pages
; first 32 bytes are on the same page
; check the first 32 bytes
	vpcmpeqb ymm1, ymm0, [rdi]
	vpmovmskb eax, ymm1
; calculate the index of the first null byte in the next 32 bytes if any
; REMIND: this is for little-endian. Use bsr instead of bsf for big-endian.
	bsf eax, eax
	jz .align_the_pointer_to_the_first_32_bytes_boundary
	ret

align 16
.first_32_bytes_are_split_on_2_pages:
...
align 16
.align_the_pointer_to_the_first_32_bytes_boundary:
	lea rax, [rdi-0x60]
	and rax, -32 ; TODO: why does `and eax, -32` trigger a segmentation fault
	NOP_8
.check_next:
; update the pointer
	add rax, 0x80
; check the next chunks of 32 bytes
	vmovdqa ymm1, [rax]
